...
public class ModerationUtils {
    ...
    private boolean isPermAvailable(ContextCommand ctx) {
        boolean hasAdmin = ctx.getAuthorMember().getPermissions()
                .stream()
                .anyMatch(perm -> perm == Permission.ADMINISTRATOR);
        logger.debug("Checking admin permission for member: " + ctx.getAuthorMember().getId() + " - Result: "
                + hasAdmin);
        return hasAdmin;
    }
    ...
    private Optional<Role> getHighestRole(List<Role> roles) {
        ...
        for (Role role : roles) {
            ...
            if (role.getPosition() > highestRole.getPosition()) {
                logger.debug("Role " + role.getName() + " (" + role.getPosition() + ") is higher than "
                        +
                        highestRole.getName() + " (" + highestRole.getPosition() + ")");
                highestRole = role;
            }
        }

        logger.info("Highest role determined: " + (highestRole != null ? highestRole.getName() : "none"));
    ...
    private boolean isRolePositionHigherToExecute(ContextCommand ctx) {
        ...
        Role memberTargetHighestRole = getHighestRole(memberTarget.getRoles())
                .orElse(null);

        Role memberAuthorHighestRole = getHighestRole(memberAuthor.getRoles())
                .orElse(null);

        logger.debug("Author highest role: "
                + (memberAuthorHighestRole != null ? memberAuthorHighestRole.getName() : "none"));
        logger.debug("Target highest role: "
                + (memberTargetHighestRole != null ? memberTargetHighestRole.getName() : "none"));
        ...
        boolean result = memberAuthorHighestRole.getPosition() > memberTargetHighestRole.getPosition();
        logger.debug("Author role position (" + memberAuthorHighestRole.getPosition()
                + ") > Target role position (" +
                memberTargetHighestRole.getPosition() + "): " + result);
        return result;
    }

    public boolean canExecute(ContextCommand ctx) {
        logger.debug("Checking if command can be executed by member: " + ctx.getAuthorId());
